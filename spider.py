...
for page in range(1, 13):   # 12个月
    if page < 10:
        url = f'http://www.tianqihoubao.com/aqi/nanjing-20210{page}.html'
        df = pd.read_html(url, encoding='gbk')[0]
        if page == 1:
            df.to_csv('2021年南京空气质量数据.csv', mode='a+', index=False, header=False)
        else:
        ...
        df = pd.read_html(url, encoding='gbk')[0]
        df.iloc[1:,::].to_csv('2021年南京空气质量数据.csv', mode='a+', index=False, header=False)
